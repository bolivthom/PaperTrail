...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  console.log('Reports Loader Debug:');
  ...
  if (receipts.length > 0) {
    console.log('First receipt:', {
      id: receipts[0].id,
      company: receipts[0].company_name,
      date: receipts[0].purchase_date,
      amount: receipts[0].total_amount.toString(),
    });
  }
  
  // If no receipts found in the period, try getting ALL receipts to debug
  if (receipts.length === 0) {
    const allReceipts = await prisma.receipt.findMany({
      where: { user_id: user.id },
      select: { id: true, purchase_date: true, company_name: true, total_amount: true },
      take: 5,
    });
    console.log('Sample of all user receipts:', allReceipts);
  }
  ...
  console.log('Spending Change:', spendingChange);
  console.log('Category Spending:', categorySpending);
  console.log('Merchant Spending:', merchantSpending);
