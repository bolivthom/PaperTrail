...
export interface ReceiptData {
  ...
  purchase_date: string; // Formatted as string from loader
  sub_total: number;
  tax_amount: number;
  total_amount: string;
  currency: string;
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const receiptData = {
    ...receipt,
    total_amount: receipt.total_amount.toNumber().toString(),
    tax_amount: receipt.tax_amount.toNumber(),
    sub_total: receipt.sub_total.toNumber(),
  };

  // Handle S3 URL
  const s3_key_parts = receipt.image_s3_url?.split('/');
  const s3_key = s3_key_parts ? s3_key_parts.slice(3, s3_key_parts.length).join('/') : undefined;
  const presignedUrl = s3_key ? await presign(s3_key) : null;

  // Format date as ISO string (no timezone conversion)
  const formattedReceipt = {
    ...receiptData,
    image_s3_url: presignedUrl,
    purchase_date: format(new Date(receipt.purchase_date), "yyyy-MM-dd"),
  };

  return json({ receipt: formattedReceipt, categories });
}
  ...
  try {
    await prisma.receipt.update({
      ...
      data: {
        category: { connect: { id: category_id } },
        company_name,
        total_amount: parseFloat(total_amount), // Convert string to number for Decimal field
        purchase_date: purchaseDate,
        notes: notes || null
      }
    });
...
export default function ReceiptDetails() {
  ...
  useEffect(() => {
    if (actionData?.success) {
      ...
      setNotification({
        show: true,
        message: ('error' in actionData && typeof actionData.error === 'string') 
          ? actionData.error 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Left Column - Form and Items */}
        <div className="space-y-6">
          <ReceiptForm receipt={receipt as unknown as ReceiptData} categories={categories} isSubmitting={isSubmitting} />
          <ReceiptItems items={receipt.items_json as any} currency={receipt.currency} />
        </div>

        {/* Right Column - Image */}
        <div>
          <ReceiptImage imageUrl={receipt.image_s3_url ?? undefined} />
        </div>
      </div>
    </div>
  );
